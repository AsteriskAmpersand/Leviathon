...
register_alias = T_ALIAS + "\s+" + parens("\$[A-V]")
register = T_REGISTER + "\s+" + parens(T_ID)
...
selective_import = T_SELECTIVE_IMPORT + "\s+" + \
    parens(T_PATH) + "\s+" + T_SELECTION + "\s+" + parens(T_ID)
qualified_import = T_QUALIFIED_IMPORT + \
    parens(T_PATH) + "\s+" + T_ALIAS + "\s+" + parens(T_ID)
...
class FandParser(Parser):
    ...
    def body(self, p):
        ...
        self.file.unindexedTargets[p.ASSIGN[0]] = (
            p.ASSIGN[0], p.ASSIGN[1], p.maybeSpecifier)
        return

    @_('ASSIGN INDEXER maybeSpecifier skip body')
    def body(self, p):
        self.file.scopeNames[p.ASSIGN[0]] = p.ASSIGN[1]
        self.file.indexedTargets[p.INDEXER] = (
            p.ASSIGN[0], p.ASSIGN[1], p.maybeSpecifier)
        return

    @_('REGISTER REGISTER_ALIAS')
    def registerDeclaration(self, p):
        self.file.registerNames[p.REGISTER] = ord(
            p.REGISTER_ALIAS[1].upper()) - ord('A')
        return
