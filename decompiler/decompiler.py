...
class THKLDecompiler(Decompiler):
    ...
    def decompile(self,entityMap=None,thkMap=None,functionResolver = None):
        ...
        if result: result+="\n"
        result += self.registerScheduler.declarations(self.settings.keepRegisters)
        self.fileNames = []
    ...
    def setupStructures(self,entityMap=None,thkMap=None,functionResolver=None):
        ...
        if self.settings.runCodeAnalysis or True:
    ...
    def labelNodes(self):
        ...
        for node in self.nodes:
            ...
            if valid or self.settings.keepVoid:
                if self.settings.forceIndex: node.explicitPosition(True)
                if self.settings.forceId: node.explicitId(True)
