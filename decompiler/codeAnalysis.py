    ...
    def targets(self, parentIndex, globaFile=None):
        ...
        for segment in self.node.segments:
            if segment.extRefThkID != -1 and globaFile is None:
                raise MissingGlobalFunction(
                    "Missing Global Node and passed external reference with ID %03d in THK_%02d" % (self.index, parentIndex))
            if segment.extRefThkID != -1:
                targets.append(
                    (globaFile.index, globaFile.find(segment.extRefNodeID)))
            if segment.localRefNodeID != -1:
                targets.append((parentIndex, segment.localRefNodeID))
        return targets

    def empty(self):
        # (self.node.segments[0].nodeEndingData // 10000) > 0
        return len(self.node.segments) == 1
...
class THKFile(RawParser):
    ...
    def __init__(self, index=0, metaHash=0, data=None, fullpath=None, settings=None):
        self.print = settings.display if settings is not None else print
    ...
    def trace(self, globaFile=None):
        ...
        for node in self.nodes:
            for source, target in node.targets(self.index, globaFile):
                self.cache_trace.add_edge(
                    (self.index, node.index), (source, target))
        return self.cache_trace
    ...
    def getActions(self, index=None):
        if index is None:
            ...
            if index < 0:
                self.print(
                    "Invalid Node ID %d Used to Access Global THK" % abs(index))
                return set()
...
def rootMatch(parentPath, childPath):
    parentTrail = list(
        reversed(list(map(lambda x: x.stem, parentPath.parents))[:-1]))
    childTrail = list(
        reversed(list(map(lambda x: x.stem, childPath.parents))[:-1]))
    root = None
    if not childTrail:
        return ""
...
class THKSummary():
    ...
    def getConnectedNodes(self, file, globallyVisited):
        visited = set([0])
        try:
            connected = nx.dfs_postorder_nodes(
                self.cache_trace, source=(file.index, 0))
        ...
        for file, fileOrphans in orphans.items():
            if fileOrphans:
                result += "%s: %s\n" % (self.resolveFile(file),
                                        ', '.join(map(lambda x: "%02d" % x, fileOrphans)))
                result += self.ssep
        return result
...
class THKProject(RawParser):
    ...
    def resolveEntity(self):
        ...
        if (self.type, self.monster, self.subspecies) in self.enums.entityMap:
            self.entity = self.enums.entityMap[(
                self.type, self.monster, self.subspecies)]
    ...
    def relativePath(self):
        pool = [rootMatch(self.path, Path(func.path))
                for func in self.thkl if func.path]
        rank = Counter(pool)
        ...
        for function in functions:
            ...
            if self.entity:
                if function.index in self.enums.moduleInfo(self.type):
                    ...
                    if function.populated:
                        path = '".\\' + index + implementationExtension + '"'
            outputStr += thklStr.function_str % (index,
                                                 path, function.metaHash) + "\n"
